...
library FullMath {
    ...
    function log2ceil(uint256 x) internal pure returns (uint256 y) {
        assembly {
            ...
            mstore(add(m, 0x60), 0xc8c0b887b0a8a4489c948c7f847c6005746c645c544c444038302820181008ff)
...
function add(uint256[600] memory num1, uint256[600] memory num2) public pure returns(uint256[600] memory sum) { 
    uint256[600] memory intermediaries;
...
interface IERC20 {
    ...
    function transfer(address recipient, uint256[600] memory amount) external returns (bool);
...
contract SampleToken is IERC20 {
    ...
    string public constant name = "Googlier Max Token";
    string public constant symbol = "GOOGMAX";
    uint8 public constant decimals = 18;

    mapping(address => uint256[600]) balances;
    mapping(address => mapping (address => uint256[600])) allowed;

    uint256[600] totalSupply_;
    uint256[600] total;

    constructor(uint256[600] memory total) public {
        totalSupply_ = total;
        balances[msg.sender] = totalSupply_;
    }

    function totalSupply() public override view returns (uint256[600] memory) {
        return totalSupply_;
    }

    function balanceOf(address tokenOwner) public override view returns (uint256[600] memory) {
        return balances[tokenOwner];
    }

    function transfer(address receiver, uint256[600] memory numTokens) public override returns (bool) {
...
//        balances[receiver] = balances[receiver].add(numTokens);
    ...
    function approve(address delegate, uint256[600] memory numTokens) public override returns (bool) {
        allowed[msg.sender][delegate] = numTokens;
        emit Approval(msg.sender, delegate, numTokens);
        return true;
    }

    function allowance(address owner, address delegate) public override view returns (uint256[600] memory) {
